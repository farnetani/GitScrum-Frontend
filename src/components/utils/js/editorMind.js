...
class sprite { // h 60 w 200
    constructor(dom, data, superior, editor) {
        ...
        this.mask = this.group.rect(0, 0).fill('transparent').move(0, 0).attr({
            'cursor': 'pointer'
        });

        this.addBtn = this.group.group().attr({
            'cursor': 'pointer'
        });
        this.addBtnC = this.addBtn.circle(15).fill('#7E84FF');
        this.addBtn.text('+').font({
            size: 16,
        }).fill('#fff').move(3, -4);
        this.addBtnMask = this.addBtn.rect(15, 15).fill('transparent').move(0, 0);
        ...
        if (this.index > 0) {
            ...
            this.removeBtn = this.group.group().attr({
                'cursor': 'pointer'
            }); // 添加
            this.removeBtnC = this.removeBtn.circle(15).fill('#F875B0'); // 添加
            this.removeBtn.text('-').font({
                size: 22,
            }).fill('#fff').move(4, -7); // 添加
