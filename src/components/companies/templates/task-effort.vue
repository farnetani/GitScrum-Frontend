...
export default {
  components: { TaskEffortCreate, TaskEffortList },
  props: {
    templateSelected: {
      type: Object,
      required: true,
  ...
  methods: {
    updateTemplateName() {
      if (this.templateSelected.name.length) {
        Axios()
          .put('templates/effort/' + this.templateSelected.id + '?company_slug=' + this.currentCompany.slug, {
    ...
    updateSelected(params) {
      Axios()
        .put('templates/effort/' + this.templateSelected.id + '/?company_slug=' + this.currentCompany.slug, params)
        .then((response) => {
...
<template>
  <div>
        ...
        <div class="template-selected-div">
          <div class="template-selected-input d-flex mb-15-px">
            <input
              ...
              class="form-control"
              :placeholder="$t('Task Effort Template Name')"
    ...
    <div class="row mt-20-px">
      <div class="col-md-12">
        <TaskEffortCreate :current-company="currentCompany" :template-selected="templateSelected"></TaskEffortCreate>
      </div>
    </div>
