...
export default {
  ...
  components: { Layout, TemplateWokflow, SideBar, TitleLoading },
  data() {
    return {
      loading: true,
      loadingCreate: false,
  ...
  methods: {
    listTemplates() {
      Axios()
        .get('templates/workflow/?company_slug=' + this.currentCompany.slug)
        .then((response) => {
    ...
    createTemplate() {
      ...
      Axios()
        .post('templates/workflow/?company_slug=' + this.currentCompany.slug, {
          name: this.name,
        })
        .then((response) => {
          this.loadingCreate = false
    ...
    deleteSelectedTemplate() {
      ...
      .then(value => {
        ...
        if(value){
          Axios()
          .delete('templates/workflow/' + this.templateSelected.id + '/?company_slug=' + this.currentCompany.slug)
          .then((_) => {
...
<template>
  <Layout>
    <div slot="content" class="template template-workflow">
        ...
        <div class="col-md-9 offset-md-1">
          ...
          <div class="row">
            ...
            <div v-if="templateSelected !== null && templateSelected.slug" class="col-md-7 offset-md-1">
              <TemplateWokflow :template-selected="templateSelected" @update-list="updateList"> </TemplateWokflow>
            </div>
          </div>
        </div>
      </div>
