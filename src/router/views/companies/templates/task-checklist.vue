...
import TemplateTaskChecklist from '@components/companies/templates/task-checklist'

export default {
  page: {
    title: 'Task Checklist Template',
    meta: [{ name: 'description', content: 'Task Checklist Template' }],
  },
  components: { Layout, TemplateTaskChecklist, SideBar, TitleLoading },
  data() {
    return {
      type: '',
      loading: true,
  ...
  methods: {
    listTemplates() {
      Axios()
        .get('templates/checklist/?company_slug=' + this.currentCompany.slug)
        .then((response) => {
    ...
    createTemplate() {
      ...
      Axios()
        .post('templates/checklist/?company_slug=' + this.currentCompany.slug, {
          name: this.name,
        })
        .then((response) => {
          this.loadingCreate = false
    ...
    deleteSelectedTemplate() {
      ...
      .then(value => {
        ...
        if(value){
          Axios()
          .delete('templates/checklist/' + this.templateSelected.id + '/?company_slug=' + this.currentCompany.slug)
          .then((response) => {
...
<template>
  <Layout>
    <div slot="content" class="template template-checklist">
